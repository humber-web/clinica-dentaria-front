          ...
          <CardContent class="p-0">
            <div class="flex flex-col">
                ...
                <div class="p-4 flex-1">
                  <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                    <div>
                      <div class="flex items-center gap-2 mb-1">
                        <component 
                          ...
                          :class="getEstadoColor(plano.estado)"
                        />
                        <h4 class="font-medium">{{ plano.descricao || `Plano de tratamento #${plano.id}` }}</h4>
                        <Badge 
                          :class="[
                            plano.estado === 'em_curso' ? 'bg-blue-100 text-blue-800 hover:bg-blue-100' :
                            plano.estado === 'concluido' ? 'bg-green-100 text-green-800 hover:bg-green-100' :
                            plano.estado === 'cancelado' ? 'bg-red-100 text-red-800 hover:bg-red-100' :
                      ...
                      <div class="flex items-center text-sm text-muted-foreground">
                        ...
                        <span>Criado em {{ formatarData(plano.data_criacao) }}</span>
                    ...
                    <div class="flex gap-2">
                      ...
                      >
                        <component :is="isExpanded(plano.id) ? ChevronUp : ChevronDown" class="mr-1 h-4 w-4" />
                        {{ isExpanded(plano.id) ? 'Ocultar' : 'Detalhes' }}
                      </Button>
                      <Button size="sm" variant="outline" @click="emit('view', plano.id)">
                      ...
                      <Button 
                        ...
                        v-if="plano.estado !== 'concluido' && plano.estado !== 'cancelado'"
                      >
              ...
              <div v-if="isExpanded(plano.id)" class="border-t bg-muted/30">
                <div class="p-4">
                  <h4 class="text-sm font-medium mb-3">Procedimentos Previstos</h4>
                  
                  <!-- No items message -->
                  <div v-if="!plano.itens?.length" class="text-sm text-center py-2 text-muted-foreground">
                    ...
                    <Table>
                      ...
                      <TableBody>
                        <TableRow v-for="item in plano.itens" :key="item.id">
                          <TableCell class="font-mono">{{ item.artigo?.codigo || `#${item.artigo_id}` }}</TableCell>
                          <TableCell>{{ item.artigo?.descricao || `Procedimento ${item.id}` }}</TableCell>
                          <TableCell>{{ item.numero_dente || '—' }}</TableCell>
                          <TableCell>{{ formatFaces(item.face) }}</TableCell>
                          <TableCell class="text-center">{{ item.quantidade_prevista }}</TableCell>
                          <TableCell class="text-center">{{ item.quantidade_executada }}</TableCell>
                          <TableCell>
                            <Badge 
                              :class="[
                                item.estado === 'concluido' ? 'bg-green-100 text-green-800 hover:bg-green-100' :
                                item.estado === 'em_andamento' ? 'bg-blue-100 text-blue-800 hover:bg-blue-100' :
                                item.estado === 'cancelado' ? 'bg-red-100 text-red-800 hover:bg-red-100' :
                                'bg-slate-100 text-slate-800 hover:bg-slate-100'
                              ]"
                            >
