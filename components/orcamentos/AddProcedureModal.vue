...
watch([selectedArtigoId, () => props.entidadeId], async () => {
  if (selectedArtigoId.value && props.entidadeId) {
    try {
      ...
      const precoInfo = await getPrecoArtigo(selectedArtigoId.value, props.entidadeId);
      if (precoInfo) {
...
watch(() => props.editItem, (item) => {
  if (item) {
    ...
    selectedFace.value = item.face || undefined;
    quantidade.value = item.quantidade;
    precoEntidade.value = typeof item.preco_entidade === 'string' 
      ? parseFloat(item.preco_entidade) 
      : item.preco_entidade;
    precoPaciente.value = typeof item.preco_paciente === 'string' 
      ? parseFloat(item.preco_paciente) 
      : item.preco_paciente;
  } else {
    resetForm(); // Agora isso funciona porque resetForm já foi definido acima
  }
}, { immediate: true });
...
<template>
  <Dialog :open="show" @update:open="onClose">
    <DialogContent class="w-[95vw] max-w-[95vw] sm:max-w-[550px] md:max-w-[600px] p-0 max-h-[90vh] flex flex-col overflow-hidden">
      <!-- Fixed Header -->
      <DialogHeader class="p-4 sm:p-6">
        <DialogTitle>{{ editMode ? 'Editar' : 'Adicionar' }} Procedimento</DialogTitle>
        <DialogDescription class="text-sm sm:text-base">
          Preencha os detalhes do procedimento a {{ editMode ? 'editar no' : 'adicionar ao' }} orçamento.
        </DialogDescription>
      </DialogHeader>
      ...
      <div class="overflow-y-auto flex-1 p-4 sm:p-6 pt-0 sm:pt-0">
        <div class="grid gap-3 sm:gap-4 py-3 sm:py-4">
          <!-- Loading state -->
          <div v-if="loading" class="flex flex-col items-center justify-center py-4 sm:py-6">
            <div class="animate-spin h-6 w-6 sm:h-8 sm:w-8 border-3 sm:border-4 border-primary border-t-transparent rounded-full mb-3 sm:mb-4"></div>
            <p class="text-sm sm:text-base text-muted-foreground">Carregando dados...</p>
          </div>
          ...
          <div v-else>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 items-start sm:items-center gap-2 sm:gap-4">
              <Label for="artigo" class="sm:text-right mb-1 sm:mb-0">Artigo</Label>
            ...
            <!-- Dente (se necessário) -->
            <div v-if="selectedArtigo && selectedArtigo.requer_dente" class="grid grid-cols-1 sm:grid-cols-4 items-start gap-2 sm:gap-4 mt-2 sm:mt-0">
              <Label class="sm:text-right mb-1 sm:mb-0 pt-0 sm:pt-2">Dente</Label>
            ...
            <!-- Face (se necessário) -->
            <div v-if="selectedArtigo && selectedArtigo.requer_face && selectedDente" class="grid grid-cols-1 sm:grid-cols-4 items-start gap-2 sm:gap-4 mt-2 sm:mt-0">
              <Label class="sm:text-right mb-1 sm:mb-0 pt-0 sm:pt-2">Face</Label>
              <div class="col-span-1 sm:col-span-3 flex justify-center sm:justify-start">
            ...
            <!-- Quantidade -->
            <div class="grid grid-cols-1 sm:grid-cols-4 items-start sm:items-center gap-2 sm:gap-4 mt-2 sm:mt-0">
              <Label for="quantidade" class="sm:text-right mb-1 sm:mb-0">Quantidade</Label>
              <div class="col-span-1 sm:col-span-3">
                <Input 
                  id="quantidade" 
            ...
            <!-- Preços unitários -->
            <div class="grid grid-cols-1 sm:grid-cols-4 items-start sm:items-center gap-2 sm:gap-4 mt-2 sm:mt-0">
              <Label class="sm:text-right mb-1 sm:mb-0">Preço Seguradora (€)</Label>
              <div class="col-span-1 sm:col-span-3">
                <Input 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 items-start sm:items-center gap-2 sm:gap-4 mt-2 sm:mt-0">
              <Label class="sm:text-right mb-1 sm:mb-0">Preço Paciente (€)</Label>
              <div class="col-span-1 sm:col-span-3">
                <Input 
      ...
      <!-- Fixed Footer -->
      <DialogFooter class="flex-col sm:flex-row gap-2 sm:gap-0 p-4 sm:p-6 border-t">
        <Button variant="outline" class="w-full sm:w-auto" @click="onClose">Cancelar</Button>
        <Button 
          @click="handleSave" 
