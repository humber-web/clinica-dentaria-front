    ...
    <Table class="w-full text-sm">
      ...
      <TableBody>
        ...
        >
          ...
          <TableCell>{{ item.artigo?.descricao || getArtigoDescricao(item.artigo_id) }}</TableCell>
          <TableCell>
      ...
      <tfoot v-if="itens.length > 0">
        <tr class="border-t font-medium">
          ...
          <td class="py-2 px-4 text-right">{{ formatCurrency(totalEntidade) }}</td>
          <td class="py-2 px-4 text-right">{{ formatCurrency(totalPaciente) }}</td>
          <td v-if="!readonly"></td>
        </tr>
      </tfoot>
    </Table>
...
const totalEntidade = computed(() => {
  return props.itens.reduce((sum, item) => {
    const value = typeof item.subtotal_entidade === 'string' 
...
const totalPaciente = computed(() => {
  return props.itens.reduce((sum, item) => {
    const value = typeof item.subtotal_paciente === 'string' 
