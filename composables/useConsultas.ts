...
export function useConsultas() {
  ...
  // Get all consultas, optionally filtered by clinica_id or medico_id
  async function fetchConsultas(clinica_id?: number, medico_id?: number) {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      consultas.value = await get(`consultas${queryString}`)
  ...
  async function createConsulta(consulta: ConsultaCreate): Promise<ConsultaRead | null> {
    loading.value = true
    error.value = null
  ...
  async function updateConsulta(id: number, changes: ConsultaUpdate): Promise<ConsultaRead | null> {
    loading.value = true
    error.value = null
    
    try {
      const data = await patch(`consultas/${id}`, changes)
      return data
    } catch (err: unknown) {
      error.value = err instanceof Error ? err.message : String(err)
  ...
  async function updateConsultaItem(
    ...
    try {
      const data = await patch(`consulta-itens/${itemId}`, changes)
      return data
    } catch (err: unknown) {
      error.value = err instanceof Error ? err.message : String(err)
  ...
  async function deleteConsultaItem(itemId: number): Promise<boolean> {
    ...
    try {
      await del(`consulta-itens/${itemId}`)
      return true
    } catch (err: unknown) {
      error.value = err instanceof Error ? err.message : String(err)
