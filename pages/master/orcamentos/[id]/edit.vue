...
const temItens = computed(() => {
  return orcamento.value && orcamento.value.itens && orcamento.value.itens.length > 0;
});
...
const getTotalGeral = (): number => {
  ...
  const entidadeValue = typeof orcamento.value.total_entidade === 'string' 
    ? parseFloat(orcamento.value.total_entidade)
    : (orcamento.value.total_entidade || 0);
    
  const pacienteValue = typeof orcamento.value.total_paciente === 'string'
...
async function updateOrcamentoField(field: keyof UpdateOrcamentoDTO, value: any) {
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <Badge :variant="getEstadoVariant(orcamento.estado)" class="text-sm">
          <div class="flex items-center gap-1">
            ...
            <CircleIcon v-if="orcamento.estado === 'rascunho'" class="w-3 h-3" />
      ...
      <div class="p-6 rounded-lg shadow mb-6 border">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            ...
            <Select 
              v-model="pacienteId"
              :disabled="temItens || orcamento.estado !== 'rascunho' || loadingPacientes"
              @update:modelValue="updateOrcamentoField('paciente_id', $event)"
            >
              <SelectTrigger>
                <SelectValue :placeholder="loadingPacientes ? 'Carregando...' : getNomePaciente(pacienteId)" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem 
          ...
          <div>
            ...
            <Select 
              v-model="entidadeId"
              :disabled="temItens || orcamento.estado !== 'rascunho' || loadingEntidades"
              @update:modelValue="updateOrcamentoField('entidade_id', $event)"
            >
              <SelectTrigger>
                <SelectValue :placeholder="loadingEntidades ? 'Carregando...' : getNomeEntidade(entidadeId)" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem 
      ...
      <div class="p-6 rounded-lg shadow mb-6 border">
        <div class="grid grid-cols-3 gap-4">
          <div class="text-right">
            ...
            <p class="text-xl font-bold">{{ formatCurrency(orcamento.total_entidade || 0) }}</p>
          </div>
          <div class="text-right">
            <p class="">Total Paciente:</p>
            <p class="text-xl font-bold">{{ formatCurrency(orcamento.total_paciente || 0) }}</p>
          </div>
          <div class="text-right">
            <p class="">Total Geral:</p>
            <p class="text-xl font-bold">{{ formatCurrency(getTotalGeral()) }}</p>
          </div>
        </div>
      </div>
